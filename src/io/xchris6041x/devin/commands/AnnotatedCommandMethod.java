...
class AnnotatedCommandMethod implements CommandMethod {
     ...
     */
    public static AnnotatedCommandMethod build(Commandable commandable, Method m) throws DevinException {
        ...
        if (m.getReturnType() != CommandResult.class)
        ...
        for (int i = 1; i < m.getParameters().length; i++) {
            ...
            if (!ObjectParsing.parserExistsFor(m.getParameters()[i].getType()))
                throw new DevinException("No parser exists for " + paramType.getCanonicalName() + ".");
        ...
        // Build CommandMethod.
        AnnotatedCommandMethod cm = new AnnotatedCommandMethod();
